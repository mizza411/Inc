...
def section_id_for_export(destination_id: str | None) -> str:
    """Map legacy inbox filings into the Other section."""
...
def _filed_by_section(queue: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for item in queue.get("items", []):
        ...
        if not dest or dest == "stay_in_chrome":
            continue
        by_section[section_id_for_export(dest)].append(item)
    return by_section
...
def export_master_document(
    ...
    """
    Rebuild the single master markdown file with ## sections per category.
    Returns (total filed link count, master path).
    """
    ...
    lines = [
        ...
        "_One document — sections match review categories. Source of truth: queue.json._",
        "",
    ]
